...
class CMContent extends CObject implements IHasSQL, ArrayAccess {
  ...
  public function Init() {
    try {
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('hello-world', 'post', 'Hej hej!', "Ett litet test.\n\n(En ny rad)", 'plain', $this->user['id']));
      $this->db->ExecuteQuery(self::SQL('insert content'), array('hello-world-again', 'post', 'Hej igen!', "Ytterligare ett test.\n\nOch en ny rad igen.", 'plain', $this->user['id']));
      $this->db->ExecuteQuery(self::SQL('insert content'), array('hello-world-once-more', 'post', 'Tjolahopp!', "Inget skoj skrivet...\n\nSynd va?", 'plain', $this->user['id']));
      $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "Min Me-sida: https://www.student.bth.se/~ankw11/phpmvc/mom5/main.php", 'plain', $this->user['id']));
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal [b]download-page[/b].\n\nYou can download your own copy of lydia from [url=https://github.com/mosbth/lydia]here[/url].", 'bbcode', $this->user['id']));
