...
This is the (only) place where Penny and Penny modules needs to be able to write and create files.</p>

<p>The path to your site-directory is (this is defined in <code>index.php</code>):</p>

<pre><code><?=PENNY_SITE_PATH.'/data'?></code></pre>

<?php 
$is_directory = is_dir(PENNY_SITE_PATH.'/data'); 
$is_writable  = is_writable(PENNY_SITE_PATH.'/data'); 
...
<p>Are you sure that you have the correct PENNY_SITE_PATH set in <code>index.php</code>? It is currently set to:</p>

<p><code><?=PENNY_SITE_PATH?></code></p>

<p>Try to create the directory.</p>

<blockquote>
<code>cd <?=PENNY_SITE_PATH?>; mkdir data; chmod 777 data</code>
</blockquote>
...
<a href='<?=create_url(null, 'step2')?>'>Reload and check again</a>&nbsp;&nbsp;&nbsp;
<?php if($is_directory && $is_writable): ?>
